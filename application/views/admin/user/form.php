<div class="row">
  <div class="col-md-8">
    ...
    <h4 class="mb-3">ข้อมูลผู้ใช้งานระบบ</h4>
    <hr>
    <form class="" action="" method="post">
      <input type="hidden" name="id" value="<?=isset($user['id'])?$user['id']:NULL;?>">
      <div class="form-group">
        <label for="">ชื่อ</label>
        <input type="text" class="form-control" name="firstname" value="<?=set_value('firstname',isset($user['firstname'])?$user['firstname']:NULL);?>" required>
        <small class="form-text text-muted"><?=form_error('firstname');?></small>
      </div>
      <div class="form-group">
        <label for="">นามสกุล</label>
        <input type="text" class="form-control" name="lastname" value="<?=set_value('lastname',isset($user['lastname'])?$user['lastname']:NULL);?>" required>
        <small class="form-text text-muted"><?=form_error('lastname');?></small>
      </div>
      <hr>
      <div class="form-group">
        <label for="">ชื่อผู้ใช้</label>
        <input type="text" class="form-control" name="username" <?=isset($user['username']) ? "value=".$user['username']." readonly" : '';?> onkeyup="if (/\W/g.test(this.value)) this.value = this.value.replace(/\W/g,'')" required>
